...
#[cfg(test)]
pub(crate) fn commit_batch_files(
    directory: &Path,
    ...
    let sources = contents
        .iter()
        .map(|content| BatchFileSource::Bytes(content))
        .collect::<Vec<_>>();
...
#[cfg(test)]
fn commit_batch_files_with_hook<F>(
    directory: &Path,
    manifest: &mut VaultBatchManifest,
    contents: &[&[u8]],
    hook: F,
    ...
    F: FnMut(usize, BatchCommitPoint) -> Result<(), String>,
{
    let sources = contents
        ...
        .map(|content| BatchFileSource::Bytes(content))
        .collect::<Vec<_>>();
    commit_batch_sources_with_hook(directory, manifest, &sources, hook)
}
...
mod tests {
    ...
    ) -> (PathBuf, PathBuf, Vec<BatchManifestEntryInput>, Vec<Vec<u8>>) {
        let first = directory.path().join("vault-first");
        let second = directory.path().join("vault-second");
        fs::create_dir_all(&first).expect("create first vault");
        fs::create_dir_all(&second).expect("create second vault");
        let existing = first.join("existing.md");
        fs::write(&existing, b"before").expect("write existing note");
        let next = vec![b"after".to_vec(), b"new".to_vec()];
        let entries = vec![
            BatchManifestEntryInput {
                approval_id: "approval-1".to_string(),
                vault_id: "vault-1".to_string(),
                vault_root: first.canonicalize().expect("canonical first vault"),
                relative_path: "existing.md".to_string(),
                previous_hash: Some(hash_bytes(b"before")),
                next_hash: hash_bytes(&next[0]),
            },
            BatchManifestEntryInput {
                approval_id: "approval-2".to_string(),
                vault_id: "vault-2".to_string(),
                vault_root: second.canonicalize().expect("canonical second vault"),
                relative_path: "new.md".to_string(),
                previous_hash: None,
                next_hash: hash_bytes(&next[1]),
            },
        ];
        (first, second, entries, next)
    }

    #[test]
    fn crash_between_vault_writes_is_rolled_back_from_manifest() {
        let directory = tempfile::tempdir().expect("create temp directory");
        let (first, second, entries, next) = fixture(&directory);
        let (manifest_dir, mut manifest) = prepare_batch_manifest(
            directory.path(),
            "batch-between-writes".to_string(),
            "采集",
            "vault.capture.batch.write",
            Some("task-1".to_string()),
            Some("trace-1".to_string()),
            entries,
        )
        .expect("prepare manifest");
        let content = next.iter().map(Vec::as_slice).collect::<Vec<_>>();
        let error =
            commit_batch_files_with_hook(&manifest_dir, &mut manifest, &content, |index, point| {
                if index == 1 && point == BatchCommitPoint::BeforeWrite {
                    Err("injected crash".to_string())
                } else {
                    Ok(())
                }
            })
            .expect_err("inject crash between writes");
        assert!(error.contains("injected crash"));
        assert_eq!(fs::read(first.join("existing.md")).unwrap(), b"after");
        let summary = recover_batch_manifests(directory.path(), |_| Ok(()));
        assert_eq!(summary.rolled_back_batches, 1);
        assert!(summary.failures.is_empty());
        assert_eq!(fs::read(first.join("existing.md")).unwrap(), b"before");
        assert!(!second.join("new.md").exists());
    }

    #[test]
    fn crash_after_write_before_progress_is_detected_by_hash() {
        let directory = tempfile::tempdir().expect("create temp directory");
        let (first, _, entries, next) = fixture(&directory);
        let (manifest_dir, mut manifest) = prepare_batch_manifest(
            directory.path(),
            "batch-before-progress".to_string(),
            "采集",
            "vault.capture.batch.write",
            None,
            None,
        ...
        let content = next.iter().map(Vec::as_slice).collect::<Vec<_>>();
        commit_batch_files_with_hook(&manifest_dir, &mut manifest, &content, |index, point| {
            if index == 0 && point == BatchCommitPoint::AfterWriteBeforeProgress {
                Err("injected crash before progress fsync".to_string())
            } else {
                Ok(())
            }
        })
        .expect_err("inject crash before progress");
        let summary = recover_batch_manifests(directory.path(), |_| Ok(()));
        assert_eq!(summary.rolled_back_batches, 1);
        assert_eq!(fs::read(first.join("existing.md")).unwrap(), b"before");
    }
    ...
    #[test]
    fn files_committed_state_replays_audit_idempotently() {
        let directory = tempfile::tempdir().expect("create temp directory");
        let (_, _, entries, next) = fixture(&directory);
        let (manifest_dir, mut manifest) = prepare_batch_manifest(
            directory.path(),
            "batch-audit-replay".to_string(),
            "采集",
            "vault.capture.batch.write",
            None,
            None,
        ...
        .expect("prepare manifest");
        let content = next.iter().map(Vec::as_slice).collect::<Vec<_>>();
        commit_batch_files(&manifest_dir, &mut manifest, &content).expect("commit files");
        let mut audit_ids = Vec::new();
        let summary = recover_batch_manifests(directory.path(), |audit| {
            audit_ids.push(audit.id.clone());
            Ok(())
        });
        assert_eq!(summary.completed_audits, 1);
        assert_eq!(audit_ids.len(), 1);
        let second = recover_batch_manifests(directory.path(), |_| {
            panic!("committed manifest must not replay audit twice")
        });
        assert_eq!(second, BatchRecoverySummary::default());
    }

    #[test]
    fn recovery_does_not_overwrite_external_conflict() {
        let directory = tempfile::tempdir().expect("create temp directory");
        let (first, _, entries, next) = fixture(&directory);
        let (manifest_dir, mut manifest) = prepare_batch_manifest(
            directory.path(),
        ...
        fs::write(first.join("existing.md"), b"external edit").expect("write conflict");
        let summary = recover_batch_manifests(directory.path(), |_| Ok(()));
        assert_eq!(summary.rolled_back_batches, 0);
        assert_eq!(summary.failures.len(), 1);
        assert_eq!(
            fs::read(first.join("existing.md")).unwrap(),
            b"external edit"
        );
    }

    #[test]
    fn commit_does_not_overwrite_edit_after_manifest_preparation() {
        let directory = tempfile::tempdir().expect("create temp directory");
        let (first, _, entries, next) = fixture(&directory);
        let (manifest_dir, mut manifest) = prepare_batch_manifest(
            directory.path(),
            "batch-prewrite-conflict".to_string(),
            "采集",
            "vault.capture.batch.write",
            None,
            None,
        ...
        .expect("prepare manifest");
        fs::write(first.join("existing.md"), b"external edit after prepare")
            .expect("write external conflict");
        let content = next.iter().map(Vec::as_slice).collect::<Vec<_>>();
        let error = commit_batch_files(&manifest_dir, &mut manifest, &content)
            .expect_err("pre-write conflict must stop commit");
        assert!(error.contains("提交前目标已被外部修改"));
        assert_eq!(
            fs::read(first.join("existing.md")).unwrap(),
            b"external edit after prepare"
        );
    }
}
