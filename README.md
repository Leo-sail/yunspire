...
当前版本为 `0.1.2`，面向 macOS 13+ 与 Windows 10/11 x64。搜索始终保留 FTS 与可从 Vault 重建的本地特征向量，并用 RRF 融合词法与向量名次；用户还可以在模型设置中明确同意后启用外部神经 Embedding。该能力默认关闭，只发送搜索查询，以及笔记标题、相对路径、标签、Wiki Links 和最多 24,000 个规范化正文字符；未同意、未配置或供应商失败时自动回退到纯本地检索。知识库的“原生图谱”入口直接唤起 Obsidian 原生 Graph View，图谱渲染和全部操作不在云枢内重做；实体知识图谱、实体消歧和多跳查询仍未开发。
