...
export function buildInboundClassificationRequest(input = {}) {
  ...
  return {
    ...
    prompt: [
      ...
      `reply JSON 契约：{"category":"一个简洁中文分类","confidence":0到1之间的数字,"evidence":[{"kind":"只能取 ${availableEvidenceKinds.join('|')}","detail":"具体、可由输入复核的依据"}],"targetPath":"必须逐字等于 allowedTargetFolders 中一个值","tags":["分类标签"]}`,
      'confidence 必须是模型对本次分类的真实自评，不得使用固定模板值；evidence 至少一条，且不得写“综合判断”等不可复核空话。分类只是一项待用户确认的本地建议。',
      `真实分类输入：${JSON.stringify(evidencePayload)}`,
    ].join('\n\n'),
  };
}
